...
(function(){
 ...
 spawnEnemy=function(type,index,total){const m=baseSpawnEnemy(type,index,total);if(m&&m.boss2)initBoss2(m);return m;};
 applyEffect=function(m,isBuff){const before=m?.health??0;const result=baseApplyEffect(m,isBuff);if(!isBuff)addBoss2ItemHpPenalty(m,before);return result;};
 applyRandomDebuff=function(m,allowDeferred=true){const before=m?.health??0;const result=baseApplyRandomDebuff(m,allowDeferred);addBoss2ItemHpPenalty(m,before);return result;};
 ...
 boss2AuraSpeedMult=function(m){const b=b2();if(!b||!m||!m.alive)return 1;if(b.boss2Mode==='orb'&&insideAura(m,b)&&orbReaction(b)===4)return 0;if(b.boss2Mode==='orb'&&b.boss2OrbItem===3)return b.boss2OrbItemMul||1;return 1;};
 ...
 fire=function(m,target,blast=false,autoBlast=false,autoHoming=false){
  const before=bullets.length;baseFire(m,target,blast,autoBlast,autoHoming);if(bullets.length<=before)return;
  const bullet=bullets[bullets.length-1],b=b2();const playerShot=m===player||m===remotePlayer;bullet.playerShot=playerShot;
  if(playerShot){const fm=playerFireMul();if(fm!==1)m.shotCd/=fm;const pm=playerProjectileMul();if(pm!==1){bullet.vx*=pm;bullet.vy*=pm;}const special3=!!(b&&insideAura(m,b)&&b.boss2Mode==='special'&&specialReaction(b)===3);if(special3){const cur=Math.max(1,Math.hypot(bullet.vx,bullet.vy)),want=m.projectile*pm,scale=want/cur;bullet.vx*=scale;bullet.vy*=scale;bullet.r=4;bullet.blast=false;bullet.homingT=8;bullet.color='#d8f5ff';bullet.boss2VisibleInAura=true;bullet.boss2Special3=true;}else bullet.boss2VisibleInAura=false;}
  else if(m.boss2)m.shotCd*=((m.boss2RapidT||0)>0?.5:(4/3));
 };
 ...
 damageMage=function(m,damage,bullet){
  ...
  const src=bullet.owner,inside=insideAura(src,m),mode=m.boss2Mode;const special=inside&&mode==='special'?specialReaction(m):0;const orb=inside&&mode==='orb'?orbReaction(m):0;const bonus=mode==='orb'&&m.boss2OrbItem===1?1:0;let dealt=inside?1+bonus:1;
  if(special===2){m.health=Math.min(m.maxHealth,m.health+10);burst(bullet.x,bullet.y,'#58ff89',12,120);ring(m.x,m.y,'#58ff89',44);sfx('hit');return true;}
  if(special===1)reflectToPlayer(src,dealt);else if(special===3){dealt=777;src.health=Math.min(src.maxHealth,src.health+2);src.boss2FadeAlpha=1;bullet.boss2VisibleInAura=true;}else if(special===4)spawnBoss2Purple(m,bullet.x,bullet.y);else if(orb===1)reflectToPlayer(src,dealt);else if(orb===2){dealt=444;src.health=Math.min(src.maxHealth,src.health+(m.boss2OrbItem===1?2:1));}else if(orb===3)m.boss2RapidT=.25;
  const oldElite=src.eliteMult;src.eliteMult=1;const oldAuraShot=bullet.boss2AuraShot,oldAura4=bullet.boss2Aura4Normal;bullet.boss2AuraShot=false;bullet.boss2Aura4Normal=false;const result=baseDamageMage(m,dealt*1000,bullet);src.eliteMult=oldElite;bullet.boss2AuraShot=oldAuraShot;bullet.boss2Aura4Normal=oldAura4;return result;
 };

 updatePickups=function(dt){
  if(run.stage===20){for(let i=pickups.length-1;i>=0;i--){const p=pickups[i];if(p.type!=='purple')continue;for(const target of livingPlayers()){if(Math.hypot(p.x-target.x,p.y-target.y)<p.r+target.r){target.health=Math.max(0,target.health-10);burst(target.x,target.y,'#bd6bff',22,150);ring(target.x,target.y,'#bd6bff',72);shake+=4;sfx('debuff');pickups.splice(i,1);killPlayerIfNeeded(target);break;}}}}
